...
2026-05-09 이후의 실서비스 전환 기준은 별도 문서인 `ARCHITECTURE_ROADMAP.md`를 따른다. 앞으로의 핵심은 단순 기능 추가가 아니라 공고 원문 신뢰도, hard rule/soft score 분리, 추천 결과 스냅샷, 서류/규칙 버전 관리, stale 처리, 알림 피로도 제어를 통해 "왜 이 추천이 나왔고 왜 바뀌었는지"를 설명 가능한 시스템으로 만드는 것이다.
